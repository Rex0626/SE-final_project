...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // PATCH 更新成功後重新導回 team_detail.php
    if ($httpcode === 200) {
        header("Location: team_detail.php?team_id=" . urlencode($teamId));
        exit();
    } 
    else {
        $error = [
...
<body>
    <div class="container">
        ...
        <form action="./edit.php?participant_id=<?= urlencode($id) ?>&team_id=<?= urlencode($teamId) ?>" method="POST">
        ...
        <div class="text-center mt-3">
            <a href="./team_detail.php?team_id=<?= urlencode($teamId) ?>" class="btn btn-secondary">返回</a>
        </div>
